...
class Netwider_multi(nn.Module):

    def __init__(self, num_conv, num_classes=100):

        super(Netwider_multi, self).__init__()
        self.num_class = num_classes
        self.num_conv = num_conv

        self.layers, self.num_pooling, self.conv_idx, self.output_channel = make_layers_multi(self.num_conv,
                                                                                              batch_norm=True)

        image_length = 32
        image_width = 32
        self.classifier = make_classifier_multi(self.output_channel * (int(image_length / (2 ** self.num_pooling))) * (
            int(image_width / (2 ** self.num_pooling))), \
                                                self.num_class)
...
def make_layers_multi(num_conv, batch_norm):
    layers = nn.ModuleList([])
    num_pooling = 0
    layer_num = -1
    conv_indx = []
    ...
    for l in range(len(cfg)):
        ...
        # Conv
        layers.append(nn.Conv2d(input_channel, cfg_m_multi[l], kernel_size=3, padding=1))
        layer_num += 1
        conv_indx.append(layer_num)

        # BN
        if batch_norm:
            layers.append(nn.BatchNorm2d(cfg_m_multi[l]))
            layer_num += 1
...
def make_classifier_multi(in_channel, num_class):
    classifier = nn.ModuleList([
        nn.Linear(in_channel * 2, 1024, bias=True),
        nn.ReLU(inplace=True),
        nn.Dropout(),
        nn.Linear(1024, 1024, bias=True),
        nn.ReLU(inplace=True),
